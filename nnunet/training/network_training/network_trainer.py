...
class NetworkTrainer(object):
    ...
    def print_to_result_file(self, *args, also_print_to_console=True, add_timestamp=True):
        ...
        if self.result_file is None:
            maybe_mkdir_p(self.output_folder)
            timestamp = datetime.now()
            self.result_file = join(self.output_folder, "best_result_log_%d_%d_%d_%02.0d_%02.0d_%02.0d.txt" %
                                 (timestamp.year, timestamp.month, timestamp.day, timestamp.hour, timestamp.minute,
                                  timestamp.second))
